...
import { boolean, string, number, array, union, literal, type } from 'io-ts';
...
export const LELodasoftCommonModelsLeadsProviderModelsZillowLenderContactDetailsIO = type({
	...
	currentBacker: createOptionFromNullable(union([literal('FannieMae'), literal('FreddieMac'), literal('FHA')])),
	...
	desiredPrograms: createOptionFromNullable(
		array(
			union([
				...
				literal('ARM5'),
				literal('ARM7'),
				literal('HomeEquity30Year'),
				literal('HomeEquity30YearDueIn15'),
				literal('HomeEquity15Year'),
				literal('HELOC20Year'),
				literal('HELOC15Year'),
				literal('HELOC10Year'),
				literal('LowOrNoDown'),
				literal('InterestOnly'),
			]),
		),
	),
	...
	propertyType: createOptionFromNullable(
		union([
			literal('SingleFamilyHome'),
			literal('TownHouse'),
			literal('CondoFourOrFewerStories'),
			literal('CondoFiveOrMoreStories'),
			literal('Cooperative'),
			literal('MobileOrManufactured'),
			literal('Modular'),
			literal('Leasehold'),
		]),
	),
	propertyUse: createOptionFromNullable(
		union([literal('Primary'), literal('SecondaryOrVacation'), literal('InvestmentOrRental')]),
	),
	...
	veteranType: createOptionFromNullable(
		union([
			literal('RegularMilitary'),
			literal('Reserves'),
			literal('NationalGuard'),
			literal('SpouseOfRegularMilitary'),
			literal('SpouseOfReserveMilitary'),
		]),
	),
